# -*- coding: utf-8 -*-
# v1.0 Small edits by former Alpha Tim
# All credits to original developers
# Lambda and additional code from Hakamac and others
import urllib
import urllib2
import datetime
import shutil
...
Q1BiYXNl = base64.b64decode('aHR0cDovL3NpbGVudGh1bnRlci5zcnZlLmlvL2pkaC9ob21lLnR4dA==')
addon = xbmcaddon.Addon('plugin.video.silenthunter')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.silenthunter]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Silent Hunter, sorry there is an error - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Silent Hunter failed to reach the server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Silent Hunter,New source added.,5000,"+icon+")")
        if not url is None:
...
dojo_url = Decode('aHR0cDovL2hlcm92aXNpb24ueDEwaG9zdC5jb20vZG9qby9tYWluLnBocA==')
def Main_Menu1():
    OPEN = Open_Url (dojo_url)
    ...
    for url,icon,desc,fanart,name in Regex:
        if name == '[COLORred]Favourites[/COLOR]':
            ...
            addDir(name,url,201,icon,fanart,desc,'','','')
            ...
            Play1(name,url,102,icon,fanart,desc)

    setView('tvshows', 'Media Info 3')			
	
def Second_Menu1(url):
    OPEN = Open_Url(url)
    Regex = re.compile('<a href="(.+?)" target="_blank"><img src="(.+?)" style="max-width:200px;" /><description = "(.+?)" /><background = "(.+?)" </background></a><br><b>(.+?)</b>').findall(OPEN)
    for url,icon,desc,fanart,name in Regex:
    ...
    setView('tvshows', 'Media Info 3')
###############################reaper######################################################
addon_id = 'plugin.video.silenthunter'
...
Base_Url = Decode('aHR0cHM6Ly9sZXRvLmZlcmFsaG9zdGluZy5jb20vZ3JpbXcwMWYvdHIv') 
...
def Main_Menu():
    ...
    for name,url,icon,fanart,desc in Regex:
        if name == 'Favourites':
            ...
            addDir(name,url,101,icon,fanart,desc,'','','')
        elif name == 'Search':
         	pass
        else:
            Play1(name,url,102,icon,fanart,desc)
...
##############silent h search###############################################################
#aHR0cDovL3NpbGVudGh1bnRlci5zcnZlLmlvL2pkaC8=
Dialog = xbmcgui.Dialog()
search_filenames = ['list','','']
ADDON_NAME = 'Silent Hunter'
def Search_silent():
    ...
    if Search_Title == '':
		...
		for file_Name in search_filenames:
			search_URL = base64.b64decode('aHR0cDovL2hlcm92aXNpb24ueDEwaG9zdC5jb20vZG9qby8=') + file_Name + '.txt'
				...
				for name,url,icon,fanart in Regex:
					if Search_Title in name.lower():
						if 'txt' in url:
							...
							Play1(name,url,501,icon,fanart,'')
							
def Play1(name,url,mode,iconimage,fanart,description,showcontext=True,allinfo={}):
...
def getData(url,fanart):
    ...
    if isinstance(soup,BeautifulSOAP):
        ...
        if len(soup('channels')) > 0:
            ...
            addDir('search Silent Hunter','',500,'','','','','','')
            addDir('[COLORlightslategray][I]Reaper[/I][/COLOR]','',100,'http://herovision.x10host.com/reaper/icon.png','http://herovision.x10host.com/reaper/fanart.jpg','','','','')
            addDir('[COLORred]Dojo Streams[/COLOR]','',200,'http://herovision.x10host.com/dojo/icon.png','http://herovision.x10host.com/dojo/fanart.jpg','','','','')
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('Silent Hunter - Select A Source', List)
         if rNo>=0:
             rName=str(List[rNo])
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Silent Hunter,Please enable Update Common Resolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
	xbmc.executebuiltin("XBMC.Notification( [B][COLOR orange]Silent Hunter[/B] is fetching the stream[/COLOR],)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Silent Hunter','Please choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Silent', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Silent Hunter Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to The Silent Hunter Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Silent Hunter, No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Silent Hunter Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Silent Hunter Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Silent Hunter, failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Silent Hunter, please [COLOR blue]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    Big_Resolve(url)
elif mode == 100: Main_Menu()
elif mode == 101 : Second_Menu(url)
elif mode == 102 :     
    if 'youtube' in url:
...
elif mode == 200: Main_Menu1()
elif mode == 201 : Second_Menu1(url)
