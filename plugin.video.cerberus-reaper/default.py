...
import yt
####################################PARTS TO EDIT#######################################
ADDON_NAME = 'Cerberus-Reaper'           ### CHANGE THIS NEW NAME OF ADDON
addon_id = 'plugin.video.cerberus-reaper' ### CHANGE THIS TO NEW FOLDER NAME - plugin.video.***   just change 'testpiece'
Base_Url = 'http://roguemedia.x10.mx/reaper/'   ###Change this to the location of the folder/url where you will be storing all your files, must end with /
...
def Main_Menu():
    ...
    for name,url,icon,fanart,desc in Regex:
        if name == 'Favourites':
         	Menu('[COLORlightsteelblue][I]Favourites[/I][/COLOR]',url,6,icon,fanart,desc)
        elif 'php' in url:
            Menu(name,url,1,icon,fanart,desc)
        elif name == 'Search':
         	Menu('[COLORlightsteelblue][I]Search[/I][/COLOR]',url,3,icon,fanart,desc)
        else:
